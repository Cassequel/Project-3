...
function renderAirlines(airlines, limit) {
  const grid = document.getElementById('airlineGrid');
  const limited = airlines.slice(0, limit);
          ...
          imgs.forEach(img => {
            if (img.complete) {
              ...
              img.onload = img.onerror = () => {
                loaded++;
                if (loaded === imgs.length) measureRows();
              };
            }
          });
        ...
        const measureRows = () => {
          setTimeout(() => {
            requestAnimationFrame(() => {
              ...
              const limit = Math.max(cardsInTwoRows.length, 4);
